...
public class ArrearInfo {
	...
	public boolean equals(Object o) {
		if (!(o instanceof ArrearInfo)) {
			return false;
		}
		ArrearInfo p = (ArrearInfo) o;
		return p.amount == amount && p.count == count
				&& p.endDegree == endDegree && p.feeStandardID == feeStandardID
				&& p.feeType == feeType && p.inputTableId == inputTableId
				&& p.name.equals(name) && p.objectID == objectID
				&& p.objectType == objectType
				&& p.payEndDate.equals(payEndDate)
				&& p.payStartDate.equals(payStartDate) && p.price == price
				&& p.startDegree == startDegree && p.status == status;
	}
}
